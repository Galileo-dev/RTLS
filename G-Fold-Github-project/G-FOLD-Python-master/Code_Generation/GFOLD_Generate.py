...
test = 0  # are we doing a static run or a generation run?
...
def GFOLD_C_GEN(prog_flag, _s_, _v_):  # PRIMARY GFOLD SOLVER
    ...
    if not test:
        # determines tf implicitly dt = tf/N, tf = dt*N(const)
        dt = Parameter(1, 1, name='dt')
        # contains all parms_static scalar variables
        S = Parameter(1, 17, name='S')
        # contains all parms_static vect variables
        V = Parameter(3, 9, name='V')
        ...
        for n in range(0, N_tf-1):
            z0_term_[n] = S[0, sk['m_wet']] - S[0, sk['alpha']] * \
                S[0, sk['r2']] * (n) * dt.value  # see ref [2], eq 34,35,36
            z1_term_[n] = S[0, sk['m_wet']] - S[0, sk['alpha']] * \
                S[0, sk['r1']] * (n) * dt.value
    ...
    con += [x[0:3, 0] == V[:, vk['r0']]]
    con += [x[3:6, 0] == V[:, vk['v0']]]
    # don't forget to slow down, buddy!
    con += [x[3:6, N_tf-1] == V[:, vk['vf']]]

    con += [s[N_tf-1] == 0]  # thrust at the end must be zero
    # thrust direction starts straight
    con += [u[:, 0] == s[0]*np.array([1, 0, 0])]
    ...
    for n in range(0, N_tf-1):  # any t in [0,tf] maps to any n in [0,N-1]
        ...
        con += [x[3:6, n+1] == x[3:6, n] +
                (dt*0.5)*((u[:, n]+V[:, g]) + (u[:, n+1]+V[:, g]))]
        con += [x[0:3, n+1] == x[0:3, n] + (dt*0.5)*(x[3:6, n+1]+x[3:6, n])]

        con += [norm((x[0:3, n]-V[:, rf])[0:2]) - V[0, c] *
                (x[0, n]-V[0, rf]) <= 0]  # glideslope constraint
        con += [norm(x[3:6, n]) <= S[0, sk['V_max']]]  # velocity

        con += [z[n+1] == z[n] - (S[0, alpha]*dt*0.5)
                * (s[n] + s[n+1])]  # mass decreases
        # limit thrust magnitude & also therefore, mass
        con += [norm(u[:, n]) <= s[n]]
        ...
        if n > 0:
            ...
            # upper thrust bound
            con += [s[n] <= mu_2[n] * (1 - (z[:, n] - z0[n]))]
            # Ensures physical bounds on z are never violated
            con += [z[n] >= z0[n]]
            con += [z[n] <= z1[n]]
